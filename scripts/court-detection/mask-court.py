...
while i < len(images) - 1:
    ...
    # convert BGR to HSV
    imgHSV = cv2.cvtColor(resultimage, cv2.COLOR_BGR2HSV)
    print('Average color (BGR): ',np.array(cv2.mean(imgHSV[150:930,150:510])).astype(np.uint8))
    # create the Mask
    mask1 = cv2.inRange(imgHSV, np.array([110, 0, 0]), np.array([120, 200, 200]))
    mask2 = cv2.inRange(imgHSV, np.array([0, 0, 200]), np.array([255, 255, 255]))
    # inverse mask
    mask = mask1 | mask2
    mask = 255-mask
    res = cv2.bitwise_and(resultimage, resultimage, mask=mask)
    ...
    cv2.imshow("Detected Lines (in red) - Standard Hough Line Transform", res)
    cv2.waitKey(0)
