...
print(images)
court_coords = np.zeros(shape=(len(images), 8))
...
for img_ in images:
    ...
    print(splits[3:])
    court_coords[i] = splits[3:] + [0, 0, 0, 0, 0, 0, 0, 0]
    ...
    x_min = np.amin(bottom_lines[:,[0,2]])

    for i in range(1, 3):
        splint_ = int(x_min + (i*(x_max - x_min)/3))
        splint_sub = np.where((bottom_lines[:,0] < splint_) & (bottom_lines[:,2] > splint_))[0]
        y_level = int(np.median(bottom_lines[splint_sub, 4]*splint_ + bottom_lines[splint_sub, 6]))
        bot_circles[i-1,] = [splint_, y_level]

    # take median of top widthwise lines
    x_max = np.amax(top_lines[:,[0,2]])
    x_min = np.amin(top_lines[:,[0,2]])

    for i in range(1, 3):
        splint_ = int(x_min + (i*(x_max - x_min)/3))
        splint_sub = np.where((top_lines[:,0] < splint_) & (top_lines[:,2] > splint_))[0]
        y_level = int(np.median(top_lines[splint_sub, 4]*splint_ + top_lines[splint_sub, 6]))
        top_circles[i-1,] = [splint_, y_level]
    ...
    int_pt1 = line_intersect(l_left_m, l_left_b, bot_lines_m, bot_lines_b)
    int_pt2 = line_intersect(l_right_m, l_right_b, bot_lines_m, bot_lines_b)
    ...
    court_coords[i] = splits[3:] + int_pt1 + int_pt2 + int_pt3 + int_pt4
