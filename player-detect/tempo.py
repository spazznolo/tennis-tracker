...
while i < 810:
    ...
    test = cv2.adaptiveThreshold(current_frame_gray, 5, blockSize=3, adaptiveMethod=cv2.ADAPTIVE_THRESH_MEAN_C, thresholdType=cv2.THRESH_BINARY, C=1) 
    #dilate_img = cv2.dilate(test, kernel, iterations=3)

    cv2.imshow('frame diff ', current_frame_gray) 
    cv2.waitKey(0) 
    i = i+1
